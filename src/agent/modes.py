...
class PlanMode:
    ...
    def create_plan(self, task: str, callback: Callable = None) -> dict:
        plan_request = (
            f"[MODE PLAN] Crée un plan structuré pour cette tâche:
"
            f"{task}
...
"
            f'{{"plan": "description", "steps": [{{"id": 1, "description": "...", "done": false}}]}}'
        )
...
[MODE PLAN ACTIF - Réponds UNIQUEMENT en JSON avec un champ "steps"]")
    ...
    def execute_all(self, callback=None):
        if not self.current_plan or not self.current_plan.get("steps"): return []
        results = []
        for step in self.current_plan["steps"]:
            if step.get("done"): continue
            if callback: callback("plan_step", {"step": step})
...
[MODE DUAL - RÔLE: GÉNÉRATEUR] Produis une solution de haute qualité."}]
        ...
        for rnd in range(1, max_rounds + 1):
            if callback: callback("dual_round", {"round": rnd})
            if rnd == 1:
...
Améliore ta solution."})
            ...
            if resp_a.tool_calls:
                msgs_a.append({"role": "assistant", "content": resp_a.content or "", "tool_calls": [{"id": t["id"], "type": "function", "function": {"name": t["name"], "arguments": t["arguments"]}} for t in resp_a.tool_calls]})
            ...
            msgs_b.append({"role": "user", "content": f"Tâche: {task}
...
Évalue."})
            ...
            last_review = review
            if callback: callback("review_result", {"round": rnd, "approved": review.get("approved")})
            if review.get("approved"): return {"content": last_content, "rounds": rnd, "review": review, "status": "approved"}
    ...
    def _parse_review(self, content):
        ...
        if m:
            try:
                r = json.loads(m.group(1)); r.setdefault("approved", False); r.setdefault("issues", []); r.setdefault("feedback", content); return r
            except: pass
        return {"approved": any(w in content.lower() for w in ["approuv", "bon", "correct", "bien"]), "issues": [], "feedback": content}
