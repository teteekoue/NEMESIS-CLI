    ...
    if CONFIG_FILE.exists():
        try:
            with open(CONFIG_FILE) as f:
                return NemesisConfig.from_dict(json.load(f))
        except Exception as e:
            print(f"[WARN] Erreur chargement config: {e}. Utilisation config par defaut.", file=sys.stderr)
...
def load_mcp_servers():
    if MCP_CONFIG_FILE.exists():
        try:
            ...
            print(f"[WARN] Erreur chargement MCP: {e}", file=sys.stderr)
    return {}
