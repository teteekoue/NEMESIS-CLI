...
@registry.register("mcp", "Gère les serveurs MCP : /mcp add <nom> <commande> | /mcp list | /mcp remove <nom>")
def mcp_command():
    args = sys.argv[2:] # Simple lecture des arguments passés après /mcp
    # Note : Le framework de commande actuel appelle la fonction sans args. 
    # Pour une vraie interaction, on va utiliser console.input dans la fonction.
    
    console.print("\n[bold magenta]Gestion des serveurs MCP[/bold magenta]")
    console.print("1. Liste des serveurs")
    console.print("2. Ajouter un serveur (/mcp add <nom> <cmd>)")
    ...
    if choice == "1":
        ...
        if not servers:
            console.print("[yellow]Aucun serveur MCP configuré.[/yellow]")
        else:
            for name, cfg in servers.items():
                console.print(f"- [bold]{name}[/bold] : {cfg['command']}")
    elif choice == "2":
        cmd_input = console.input("Entrez 'add <nom> <commande>' : ").strip()
        parts = cmd_input.split(" ", 2)
        if len(parts) >= 3 and parts[0] == "add":
            success, msg = mcp_mgr.add_server(parts[1], parts[2])
            console.print(f"[success]{msg}[/success]" if success else f"[error]{msg}[/error]")
    elif choice == "3":
        name = console.input("Nom du serveur à supprimer : ").strip()
